  ...
  <>
   <ScrollView style={{backgroundColor: 'black'}}>
      <View style={{width: '100%',header: '100%'}}>
        ...
        {ColdWater.map(({water, id,name,price,image,quantity},index) => (
            <TouchableOpacity style={{backgroundColor: day? '#b0b0b0': '#2E3860', borderRadius: 55, marginBottom: '3%', width: '47%', padding: '2%'}} key={index} onPress={()=>{ dispatch(add(({name,price, image,id,quantity})))}}>
                <>
                <Image source={image} style={{width: '100%' ,height: 150, padding: '23%', }} />
